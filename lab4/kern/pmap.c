...
boot_alloc(uint32_t n)
{
	...
	if(n == 0) {
		...
		result = nextfree;
		if((uintptr_t)(nextfree + n) >= KERNBASE + (PGSIZE * 1024)) {
			cprintf("boot_alloc: out of memory!\n");
			panic("boot_alloc: failed to allocate %d byte.\n", n);
		}
...
page_init(void)
{
	...
	*/
	...
	for (physaddr_t pa = mpentry_pa; pa < mpentry_pa_end; pa += PGSIZE) {
	...
	for(size_t j = 0; j < io_hole_pages; j++, i++) {
		pages[i].pp_ref = 1;
		pages[i].pp_link = NULL;
	}
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	// If this page directory entry is present
	if(*(pgdir + pdx) & PTE_P) {
		physaddr_t pgtable_pa = PTE_ADDR(*(pgdir + pdx));
		pte_t *pgtable_va = (pte_t *) KADDR(pgtable_pa);
		result = pgtable_va + ptx;
	}
	// If this page directory entry is not present
	else if(create) {
		struct PageInfo *new_pt_page = page_alloc(ALLOC_ZERO);
		if(!new_pt_page) {
			cprintf("pgdir_walk: allocate a page table page failed!\n");
			return NULL;
		}
		new_pt_page->pp_ref++;
		physaddr_t new_pt_page_pa = page2pa(new_pt_page);
		*(pgdir + pdx) = new_pt_page_pa | PTE_P | PTE_U | PTE_W;
		result = (pte_t *) KADDR(PTE_ADDR(*(pgdir + pdx))) + ptx;
	}
	else
		result = NULL;
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	...
	// for( ; va < limit; va += PGSIZE, pa += PGSIZE) {
	// ...
	// The max value of uintptr_t can represent is 2^32 - 1,
	// but va + size could be equal to 2^32
	// which will cause limit equal to 0.
	for(int i = 0; i < size/PGSIZE; i++, va += PGSIZE, pa += PGSIZE) {
		pte_t *pte = pgdir_walk(pgdir, (void *) va, 1);
		assert(pte);
		if(pte) *pte = pa | PTE_P | perm;
		else panic("boot_map_region: out of memory!\n");
	}
}
...
page_insert(pde_t *pgdir, struct PageInfo *pp, void *va, int perm)
{
	// Fill this function in
	// My code:
	// Find a pointer to the PTE for linear address 'va'
	...
	if(*pte & PTE_P) {
		// If virtual address 'va' already mapped to page pp
		...
		// If virtual address 'va' mapped to other page
		// remove that page
...
page_remove(pde_t *pgdir, void *va)
{
	// Fill this function in
	// My code:
	// Find the physical page mapped at virtual address 'va'
	pte_t *pte = NULL;
	struct PageInfo *page = page_lookup(pgdir, va, &pte);

	// If there is physical page at virtual address 'va'
	if(page) {
		// decrease the ref count on the physical page
		// and free the physical page if the refcount reaches 0.
		page_decref(page);
		if(pte) *pte = 0;
	...
	// If there is no physical page at virtual address 'va'
	// Nothing to do.
}
