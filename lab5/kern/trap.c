...
trap_init_percpu(void)
{
	...
	// My code:
	struct CpuInfo *cur_cpu = thiscpu;
	struct Taskstate *cur_ts = &cur_cpu->cpu_ts;

	// Setup a TSS so that we get the right stack
	// when we trap to the kernel.
	cur_ts->ts_esp0 = KSTACKTOP - cur_cpu->cpu_id * (KSTKSIZE + KSTKGAP);
	cur_ts->ts_ss0 = GD_KD;
	cur_ts->ts_iomb = sizeof(struct Taskstate);

	// Initialize the TSS slot of the gdt.
	gdt[(GD_TSS0 >> 3) + cur_cpu->cpu_id] = SEG16(STS_T32A,
			(uint32_t) (cur_ts), sizeof(struct Taskstate) - 1, 0);
	gdt[(GD_TSS0 >> 3) + cur_cpu->cpu_id].sd_s = 0;

	// Load the TSS selector (like other segment selectors, the
	// bottom three bits are special; we leave them 0)
	ltr(GD_TSS0 + (cur_cpu->cpu_id << 3));
